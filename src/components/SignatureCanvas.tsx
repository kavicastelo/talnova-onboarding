...
export const SignatureCanvas: React.FC<SignatureCanvasProps> = ({ onSave, defaultSignerName = '' }) => {
  ...
  const handleConfirm = () => {
    if (!signerName.trim()) {
      alert(t('signature.enterLegalNameAlert', 'Please enter your full legal name.'));
      return;
    }
    ...
    if (activeTab === 'draw') {
      const canvas = canvasRef.current;
      if (!canvas || !hasDrawn) {
        alert(t('signature.drawSignatureAlert', 'Please draw your signature on the canvas.'));
        return;
      }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex border-b text-sm font-medium">
        ...
        >
          <PenTool className="h-4 w-4" /> {t('signature.drawSignature', 'Draw Signature')}
        </button>
        <button
          className={`py-2 px-4 border-b-2 flex items-center gap-2 transition-colors ${
            activeTab === 'type'
        ...
        >
          <Type className="h-4 w-4" /> {t('signature.typeSignature', 'Type Signature')}
        </button>
      </div>
      ...
      <div>
        <label className="text-xs font-semibold text-muted-foreground block mb-1">
          {t('signature.legalNameLabel', 'Full Legal Name (Required)')}
        </label>
        <Input
          placeholder={t('signature.legalNamePlaceholder', 'e.g. John Doe')}
          value={signerName}
      ...
      {activeTab === 'draw' ? (
        <div className="space-y-2">
          <div className="border-2 border-dashed rounded-lg bg-card p-2 relative text-center">
            ...
            <div className="text-[11px] text-muted-foreground mt-1">
              {t('signature.canvasHelp', 'Draw your signature above using mouse or touch')}
            </div>
          </div>
          <div className="flex justify-end">
            <Button variant="ghost" size="sm" onClick={clearCanvas} className="text-xs text-muted-foreground">
              <Eraser className="h-3.5 w-3.5 mr-1" /> {t('signature.clearCanvas', 'Clear Canvas')}
            </Button>
          </div>
        </div>
      ) : (
        <div className="border-2 border-dashed rounded-lg p-6 bg-white text-center">
          <div className="text-3xl font-serif italic text-slate-800 tracking-wide select-none py-4">
            {signerName || t('signature.previewPlaceholder', 'Your Signature Preview')}
          </div>
          <p className="text-xs text-muted-foreground">
            {t('signature.typedVerificationNotice', 'Typed signature formatted with legal timestamp verification.')}
          </p>
        </div>
      )}
      ...
      <div className="pt-2 flex justify-end gap-2">
        <Button className="bg-indigo-600 hover:bg-indigo-700 text-white w-full sm:w-auto" onClick={handleConfirm}>
          {t('signature.applySignature', 'Apply E-Signature')}
        </Button>
      </div>
    </div>
  );
